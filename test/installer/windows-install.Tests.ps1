...
Describe 'Windows installer (live)' -Skip:(-not $script:live) {
    ...
    It 'rejects a checksum mismatch with exit 6 and installs nothing' {
        ...
        $r = Invoke-Script $script:install @('-Yes', '-Manifest', $bad, '-ArtifactDir', $script:dist, '-PgPassFile', $script:pass, '-ServiceName', $script:svc, '-Database', $script:db)
    ...
    It 'installs, restarts, activates and passes doctor' {
        $r = Invoke-Script $script:install @('-Yes', '-Manifest', $script:manifest, '-ArtifactDir', $script:dist, '-PgPassFile', $script:pass, '-ServiceName', $script:svc, '-Database', $script:db)
    ...
    It 'is idempotent on rerun (no restart)' {
        $r = Invoke-Script $script:setup @('install', '-Yes', '-PgPassFile', $script:pass, '-ServiceName', $script:svc, '-Database', $script:db)
        $r.ExitCode | Should -Be 0 -Because $r.Output
