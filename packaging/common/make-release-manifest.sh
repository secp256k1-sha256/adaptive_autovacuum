...
for f in "$dist"/*.deb "$dist"/*.rpm "$dist"/*.zip; do
    ...
    case "$name" in
        *-debuginfo-*|*-debugsource-*|*-dbgsym_*) echo "skipping debug package $name" >&2; continue ;;
        ...
        # adaptive-autovacuum-setup_1.1.0-1_all.deb : helper shared by every major and Ubuntu release
        ...
        # postgresql-18-adaptive-autovacuum_1.1.0-1_ubuntu24.04_amd64.deb
        ...
        # postgresql18-adaptive-autovacuum-1.1.0-1.el9.x86_64.rpm
        ...
        # adaptive_autovacuum-1.1.0-pg18-windows-x64.zip
...
jq -n --arg v "$version" --arg t "$tag" --argjson a "$artifacts" \
   '{schema_version:1, extension_version:$v, package_revision:1, minimum_installer_version:"1.1.0", published_at:(now|todate), release_tag:$t, artifacts:$a}' \
