...
- Repair `autovacuum = off`, the single most dangerous misconfiguration for an unattended installation, after it has been observed for a configurable number of consecutive checks.
...
The launcher is registered from `_PG_init()` while the library is loaded through `shared_preload_libraries`. It connects to the configured control database, reads the shared `pg_database` catalog, and starts dynamic database workers - up to `adaptive_autovacuum.max_database_workers` concurrently (default 2; 1 gives a strictly serial scan). One full pass over all databases is followed by the `naptime_seconds` sleep, so the revisit period of a database is the pass runtime plus the naptime, not the naptime alone.
...
The launcher lists databases in descending transaction age (`GREATEST(age(datfrozenxid), mxid_age(datminmxid))`), so the databases nearest wraparound are inspected first in every sweep. `adaptive_autovacuum.max_database_workers` bounds a slot scheduler in the launcher: up to that many database workers run concurrently, each still under its own `database_worker_timeout_seconds`. The default is 2: long-running emergency vacuums already run outside these slots in the dedicated emergency worker (4.2b), but the routine cycle itself can still be slow in one database (a large policy scan, lock waits, or the in-cycle ANALYZE of a big never-analyzed table), and with a second slot that never delays the checks - including the wraparound checks - of the databases scheduled after it. Setting 1 gives the strictly serial scan (lowest controller concurrency, easiest to reason about).
...
- Start and supervise up to `max_database_workers` dynamic workers (default 2).
...
- `emergency_queue`: guarded manual-vacuum requests, each carrying the projected seconds to the read-only cutoff at enqueue time (`deadline_seconds`); the worker claims by deadline first.
- `controller_state`: one row of per-cycle samples - XID and WAL counters for the rates, the previous maintenance-debt total and its smoothed velocity, the backlog-free and autovacuum-off cycle counters, and the operator baseline of the cost settings that the decay walks back toward.
...
- **Fixed allowlist**, enforced twice: the SQL policy only enqueues the twelve managed maintenance GUCs plus the repair-only `autovacuum` entry, and the C applier independently rejects anything outside its compiled-in list.
- **`autovacuum = off` repair.** With `repair_disabled_autovacuum` (default true) the policy counts consecutive checks that saw `autovacuum` off in `controller_state` and, at `repair_disabled_autovacuum_cycles` (default 10), queues `autovacuum = on` through the same audited path; the applier accepts no other value for that GUC, so the extension can switch autovacuum on but never off. The persistence requirement keeps a deliberate short maintenance window from being undone within a minute; an installation that intentionally runs without autovacuum sets the flag to false.
...
Cluster GUCs affect the whole cluster, so they are decided from cluster-wide evidence. After every policy cycle the database worker publishes a compact summary into a shared-memory slot array sized by `adaptive_autovacuum.max_tracked_databases` (default 256, postmaster context; one slot per database). Only slots older than ten naptimes are ever reused; when no slot is free the summary is dropped, a warning is logged once per worker, `cluster_summary_status()` reports the overflow, and every database's cycle sees `evidence_complete = false` in the aggregate, which turns cluster-wide changes into record-only recommendations until the overflow is ten naptimes old. The summary carries: eligible and overdue relation counts, dead- and insert-overdue counts, the fleet-derived trigger-ceiling target, the medians of the per-relation desired trigger settings, and the database's maintenance-debt total and smoothed velocity. Before the next cycle in any database, the C worker sums the OTHER databases' fresh slots (slots older than ten naptimes are ignored, so dropped or disabled databases age out) and hands the aggregate to `_run_cycle` as jsonb. The SQL policy merges it with its own fresh numbers: counts add up, the trigger ceiling takes the cluster-wide maximum, and cross-database medians are approximated as overdue-count-weighted averages of the per-database medians. The mistuned-baseline detectors, the worker-count recommendation, and the overdue-driven cost branch all run on the merged values; the recommendation reason states the cluster evidence explicitly. The merge warms up one sweep after a restart (the first cycle of each database sees no summaries yet and behaves database-locally).

Three rails bound the apply side: the shared per-GUC once-per-two-naptimes cooldown (always on), deduplication with no-op filtering, and `adaptive_autovacuum.global_settings_database`, which names one database as the sole queuer and applier - the recommended configuration when audit locality matters. Audit rows land in the database whose worker acted.
...
5. Optionally set `dry_run = true` in the database policy (a fresh install is active); enable the cluster GUC and reload.
6. Observe several cycles: review `decisions`, `global_recommendations`, and proposed values against real churn.
7. Turn dry-run off if it was set. Cluster-setting management activates here - set `manage_global_settings = false` first if cluster changes must stay manual.
...
- True cross-database median computation for the mistuned-baseline detector (today: overdue-count-weighted average of per-database medians).
...
| Worker pool not capped by CPU count | Shared cost budget means workers add concurrency, not proportional I/O; small hosts with long overdue queues need more than one worker per CPU | Relies on the load-per-CPU gate, memory cap and `recommendation_workers_max` for CPU safety |
...
| Cluster GUCs from shared-memory summary aggregation | Settings affect the whole cluster; one database's tables are not the cluster | Weighted-median approximation; one-sweep warm-up after restart |
